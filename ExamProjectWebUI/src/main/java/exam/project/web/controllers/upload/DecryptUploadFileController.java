...
@Controller
// Class
public class DecryptUploadFileController {

    @GetMapping("crypt/decrypt/upload") public String upload()
    {
        return "fileform";
    }

    @RequestMapping(value = "crypt/decrypt/uploadfile",
               ...
               Model mod)
    {
        ...
        try {
            ...
            cryptConverter.decrypt(filePath);
            Pattern pattern = Pattern.compile("([\\w:\\\\]+)[\\\\](\\w+)[.]([a-z]+)$");
            Matcher matcher = pattern.matcher(filePath);
            String newFile = "";
            while (matcher.find())
                newFile = matcher.group(2) + "_decrypted." + matcher.group(3);
